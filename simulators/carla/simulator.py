...
class CarlaSimulation(DrivingSimulation):
    def __init__(self, scene, client, tm, timestep, render, record, scenario_number, verbosity=0):
        ...
        if self.render:
            # change camera display dim
            self.displayDim = (320, 180)
        ...
        for obj in self.objects:
            ...
            if obj is self.objects[0]:
                ...
                if self.render:
                    camIndex = 0
                    camPosIndex = 1
                    self.cameraManager = visuals.CameraManager(self.world, carlaActor, self.hud, camPosIndex)
                    self.collisionManager = visuals.CollisionSensor(self.world, carlaActor, self.hud)
                    self.bevManager = visuals.BevManager(self.world, carlaActor, self.client)
    ...
    def destroy(self):
        ...
        if self.render and self.cameraManager:
            ...
            self.collisionManager.destroy_sensor()
            self.bevManager.destry()
